...
int identical(BTNode *tree1, BTNode *tree2)
{
    ...
    { // if one tree empty and the other is not empty
        return 0;
    }

    l = identical(tree1->left, tree2->left);   // recurisve call down
    r = identical(tree1->right, tree2->right); // recurisve call down
    if (l & r & tree1->item == tree2->item)
    { // return condition
        return 1;
    }
    else
